...
RegisteredSecurities() %>% head(10) %>% kable() %>% kable_styling()
...
getMarketIndex(sector = "finance",
               from = "01.01.2022",
               to = "30.06.2025") %>% head(10) %>% kable() %>% kable_styling()
...
getTicker("UZ7011340005", 
          from = "01.01.2022",
          to = "30.06.2025") %>% head(10) %>% select(-9, -10) %>% kable() %>% kable_styling()
...
# Baseline prices
Rb <- getMarketIndex("all", from = "01.01.2023", to = "01.08.2025")  %>%
...
RaRb_capm <- RaRb %>%
  ...
  tq_performance(
    Ra             = Ra,
    Rb             = Rb,
    Rf             = 0.15, # Risk-free rate of return (15% per annum)
    performance_fun = table.CAPM
  )
